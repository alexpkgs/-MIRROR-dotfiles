...
local sectors_data = {
    { start_angle = 0, end_angle = math.pi / 2, icon = "globe", callback = function() 
        awful.spawn("firefox")
    end },
    { start_angle = math.pi / 2, end_angle = math.pi, icon = "code", callback = function() 
        awful.spawn("emacs")
    end },
    { start_angle = math.pi, end_angle = 3 * math.pi / 2, icon = "terminal", callback = function() 
        awful.spawn("alacritty")
    end },
    { start_angle = 3 * math.pi / 2, end_angle = 2 * math.pi, icon = "edit", callback = function() 
...
for _, sector in ipairs(sectors_data) do
    icons[sector.icon] = gears.surface.load_uncached("/home/nuxsh/dotfiles/config/awesome/icons/" .. sector.icon .. ".svg")
end
...
local function create_sector_widget(sector)
    local widget = wibox.widget {
        ...
        draw = function(self, _, cr, width, height)
            ...
            cr:set_source(gears.color(beautiful.bg_normal))
            cr:fill_preserve()

            cr:set_source(gears.color(beautiful.border_color))
...
local function hide_menu()
    if menu.is_visible then
        ...
        if menu.anim_timer then menu.anim_timer:stop() end
...
menu_wibox:buttons(gears.table.join(
    awful.button({}, 1, function(_, x, y)
